            ...
            try
            {
                ...
                if (bFirstPage)
                {
                    foreach (object obj in dataGridView1.Columns)
                    {
                        ...
                        int iTmpWidth = (int)Math.Floor(GridCol.Width / (double)iTotalWidth * iTotalWidth * (e.MarginBounds.Width / (double)iTotalWidth));
                ...
                while (iRow <= dataGridView1.Rows.Count - 1)
                {
                    ...
                    if (bNewPage)
                    {
                        e.Graphics.DrawString("Cleaning summary for " + userName, new Font(dataGridView1.Font, FontStyle.Bold), Brushes.Black, e.MarginBounds.Left, e.MarginBounds.Top - e.Graphics.MeasureString("Cleaning summary for " + userName, new Font(dataGridView1.Font, FontStyle.Bold), e.MarginBounds.Width).Height - 13f);
                        e.Graphics.DrawString("Date: " + headerDate, new Font(dataGridView1.Font, FontStyle.Bold), Brushes.Black, e.MarginBounds.Left + (e.MarginBounds.Width - e.Graphics.MeasureString("Date: " + headerDate, new Font(dataGridView1.Font, FontStyle.Bold), e.MarginBounds.Width).Width), e.MarginBounds.Top - e.Graphics.MeasureString("Cleaning summary for " + userName, new Font(new Font(dataGridView1.Font, FontStyle.Bold), FontStyle.Bold), e.MarginBounds.Width).Height - 13f);
                        ...
                        foreach (object obj2 in dataGridView1.Columns)
                        {
                            ...
                            e.Graphics.DrawString(GridCol2.HeaderText, GridCol2.InheritedStyle.Font, new SolidBrush(GridCol2.InheritedStyle.ForeColor), new RectangleF((int)arrColumnLefts[iCount], iTopMargin, (int)arrColumnWidths[iCount], iHeaderHeight), strFormat);
                            iCount++;
                        }
                        bNewPage = false;
                    ...
                    foreach (object obj3 in GridRow.Cells)
                    {
                        ...
                        if (Cel.Value != null)
                        {
                            e.Graphics.DrawString(Cel.Value.ToString(), Cel.InheritedStyle.Font, new SolidBrush(Cel.InheritedStyle.ForeColor), new RectangleF((int)arrColumnLefts[iCount], iTopMargin, (int)arrColumnWidths[iCount], iCellHeight), strFormat);
                        }
